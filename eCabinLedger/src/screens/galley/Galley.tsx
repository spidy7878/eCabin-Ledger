...
export default function Galley() {
  ...
  useEffect(() => {
    ...
    api.getGalleys(selectedAircraft.AircraftId)
      .then((data) => {
        ...
        const unique = data.filter((g) => { if (seen.has(g.GalleyCode)) return false; seen.add(g.GalleyCode); return true; });
        setGalleys(unique);
  ...
  useEffect(() => {
    if (!activeGalley || !selectedAircraft || !activeGalley.SubCatID) { setItems([]); return; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      startSync().catch(() => {});
      if (isWorkflow) {
        Alert.alert("Galley Saved ✓", `${count} photo${count > 1 ? "s" : ""} queued. Continue to Lavatory?`, [
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F7F9FB" }}>
          ...
          {loadingIssues ? <View style={{ padding: 24, alignItems: "center" }}><ActivityIndicator color={colors.primary} /></View> : (
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <TouchableOpacity onPress={() => { setSelectedIssue(item); setIssueModalVisible(false); }} style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 14, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: "#F3F4F6", backgroundColor: isSelected ? "#EEF2FF" : "white" }}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: 140 + insets.top, paddingHorizontal: spacing.md, paddingBottom: (insets.bottom || 0) + 100 }}>
        ...
        {loadingGalleys ? <LoadingRow /> : galleys.length === 0 ? (
          <Text style={{ color: "#6B7280", marginBottom: 16, fontSize: 12 }}>No galleys found.</Text>
        ) : (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
            {galleys.map((g) => <Pill key={g.GalleyId} label={g.GalleyCode} active={activeGalley?.GalleyId === g.GalleyId} onPress={() => setActiveGalley(g)} />)}
          </ScrollView>
        )}
        {activeGalley && (
          <Text style={{ fontSize: 11, color: "#6B7280", marginBottom: 4 }}>
            {activeGalley.GalleyName} · {activeGalley.Location} · Status:{" "}
            <Text style={{ color: activeGalley.Status === "Operational" ? colors.success : colors.danger }}>{activeGalley.Status}</Text>
          </Text>
        )}
        ...
        {loadingItems ? <LoadingRow /> : (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
            ...
            {!loadingItems && items.length === 0 && activeGalley && <Text style={{ fontSize: 12, color: "#9CA3AF" }}>No items for this galley</Text>}
          </ScrollView>
        )}
