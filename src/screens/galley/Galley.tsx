...
export default function Galley() {
  ...
  useEffect(() => {
    if (!activeItem || !activeGalley || !selectedAircraft) {
      setSubmittedImages([]);
      return;
    }
    getImagesForItem(
  ...
  }, [activeItem, activeGalley?.GalleyId, selectedAircraft?.AircraftId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Auto-advance: jump to the next sub-item in this galley; when its items
      // are exhausted, move to the next galley (whose first item is auto-selected
      // by the parts effect).  Advance silently — only prompt to move to the next
      // screen once every galley & item here is done.
      ...
      const zoneIdx = galleys.findIndex((g) => g.GalleyId === activeGalley?.GalleyId);
      if (zoneIdx >= 0 && zoneIdx < galleys.length - 1) {
        setActiveGalley(galleys[zoneIdx + 1]);
        return;
      }

      // Last item — reload submitted so button flips to "Submitted ✓"
      getImagesForItem(
        selectedAircraft.AircraftId, "galley", activeGalley.GalleyId, activeItem
      ).then(setSubmittedImages).catch(() => {});
    ...
    <View style={{ flex: 1, backgroundColor: "#F7F9FB" }}>
      ...
      >
        ...
        {loadingGalleys ? (
          ...
          <Text style={{ color: "#6B7280", marginBottom: 16, fontSize: 12 }}>No galleys found for {registration}.</Text>
        ) : (
        ...
        <ZoneProgressBar done={progress.galley.done} total={progress.galley.total} label="Galley Progress" />
